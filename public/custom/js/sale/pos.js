...
smartAddItem(item);  // இப்போ duplicate வராது!
            }
        });
    });
...
/**
 * Master Function - Smart Add Item (Grid + Search இரண்டுக்கும் பொதுவா)
 * Already இருந்தா quantity +1, இல்லைனா new row
 */
function smartAddItem(itemData) {
    let existingRowId = null;

    $('#invoiceItemsTable tbody tr').each(function () {
        const rowId = $(this).attr('id');
        if (!rowId || $(this).hasClass('default-row')) return true;

        const itemIdInput = $(this).find(`input[name="item_id[${rowId}]"]`);
        if (itemIdInput.length && parseInt(itemIdInput.val()) === parseInt(itemData.id)) {
            existingRowId = rowId;
            return false;
        }
    });
    ...
    if (existingRowId !== null) {
        ...
        $('html, body').animate({ scrollTop: row.offset().top - 200 }, 400);
    } else {
        // Prepare proper data object
        const dataObject = {
            warehouse_id: itemData.warehouse_id || $('#warehouse_id').val(),
            id: itemData.id,
            name: itemData.name,
            brand_name: itemData.brand_name || '',
            tracking_type: itemData.tracking_type || 'regular',
            description: itemData.description || '',
            sale_price: itemData.sale_price,
            is_sale_price_with_tax: itemData.is_sale_price_with_tax ? 1 : 0,
            tax_id: itemData.tax_id || null,
            quantity: 1,
            taxList: itemData.taxList || [],
            unitList: itemData.unitList || [],
            base_unit_id: itemData.base_unit_id,
            selected_unit_id: itemData.selected_unit_id || itemData.base_unit_id,
            conversion_rate: itemData.conversion_rate || 1,
            sale_price_discount: itemData.sale_price_discount || 0,
            discount_type: itemData.sale_price_discount_type || 'fixed',
            mrp: itemData.mrp || 0,
        };
...
/**
 * Calculate Sum Of Total Column(s) + MRP Discount + Total Save
 * */
function setSumOfTablefooter(){
    let sumOfTotalColumn = 0;
    let sumOfQuantityColumn = 0;
    let sumOfTaxColumn = 0;
    let sumOfDiscountColumn = 0;
    let sumMRPDiscount = 0; // <-- New: Total MRP Discount

    $('#invoiceItemsTable tbody tr').each(function () {
        const rowId = $(this).attr('id');
        if (!rowId || $(this).hasClass('default-row')) return; // skip empty

        const quantity = returnDecimalValueByName('quantity[' + rowId + ']') || 0;
        const salePrice = returnDecimalValueByName('sale_price[' + rowId + ']') || 0;
        const mrp = returnDecimalValueByName('mrp[' + rowId + ']') || 0;
        const discountAmount = returnDecimalValueByName('discount_amount[' + rowId + ']') || 0;
        const taxAmount = returnDecimalValueByName('tax_amount[' + rowId + ']') || 0;
        const total = returnDecimalValueByName('total[' + rowId + ']') || 0;

        // MRP Discount per row = (MRP - Sale Price) × Quantity (only if MRP > Sale Price)
        const mrpDiscountPerRow = (mrp > salePrice) ? (mrp - salePrice) * quantity : 0;
        sumMRPDiscount += mrpDiscountPerRow;

        sumOfQuantityColumn += quantity;
        sumOfDiscountColumn += discountAmount;
        sumOfTaxColumn += taxAmount;
        sumOfTotalColumn += total;
    });

    // Update all footer values
    $('.sum_of_quantity').text(_parseQuantity(sumOfQuantityColumn));
    $('.sum_of_discount').text(_parseFix(sumOfDiscountColumn));
    $('.sum_of_tax').text(_parseFix(sumOfTaxColumn));
    $('.sum_of_total').text(_parseFix(sumOfTotalColumn));

    // New: Update MRP Discount
    $('.discount_of_mrp').text(_parseFix(sumMRPDiscount));
